...
namespace ljk
{
    public class BeeSimulation : MonoBehaviour
    {
        ...
        [Header("地面避免参数")]
        public float groundAvoidanceHeight = 3f;        // 最小飞行高度
        public float groundAvoidanceWeight = 8f;        // 地面避免力权重
        public LayerMask groundLayerMask = 1;           // 地面层掩码
        ...
        void FixedUpdate()
        {
            ...
            Vector3 groundAvoidanceForce = CalculateGroundAvoidanceForce();

            // 检查悬停状态
            bool isHovering = beeTarget.IsHovering;

            Vector3 totalForce = Vector3.zero;

            // 地面避免最高优先级
            if (groundAvoidanceForce.magnitude > 0.1f)
            {
                totalForce = groundAvoidanceForce * 2f +
                            targetSeekForce * 0.3f +
                            oscillationForce * 0.2f;
            }
            // 障碍物避免
            else if (obstacleAvoidanceForce.magnitude > 0.1f)
            {
                ...
                totalForce = avoidanceWeight * obstacleAvoidanceForce +
                            oscillationForceWeight * oscillationForce * 0.3f +
                            targetSeekForce * 0.3f +
                            curlNoiseWeight * curlNoiseForce * 0.3f;
            }
            // 悬停状态的特殊处理
            else if (isHovering)
            {
                // 悬停时使用简化的力组合
                totalForce = targetSeekForce * 1.5f +      // 悬停力为主要力
                            oscillationForce * 0.1f +      // 轻微振荡保持自然
                            curlNoiseForce * 0.1f;         // 少量噪声

                // 悬停时添加额外的速度阻尼
                velocity *= 0.98f;

                // 限制悬停时的最大速度
                maxSpeed = Mathf.Min(originalMaxSpeed, beeTarget.hoverSpeed * 2f);
            }
            else
            {
                // 正常飞行
            ...
            // 限制垂直方向力
            Vector3 horizontalForce = Vector3.ProjectOnPlane(totalForce, Vector3.up);
            Vector3 verticalForce = Vector3.Project(totalForce, Vector3.up);

            float maxVerticalForce = isHovering ? maxForce * 0.2f : maxForce * 0.3f;
            if (verticalForce.magnitude > maxVerticalForce)
            {
                verticalForce = verticalForce.normalized * maxVerticalForce;
            }
            ...
            if (isHovering)
            {
                ...
                if (currentSpeed > beeTarget.hoverSpeed * 1.5f)
                {
                    velocity = velocity.normalized * (beeTarget.hoverSpeed * 1.5f);
                }

                // 悬停时更强的阻尼
                velocity = Vector3.Lerp(velocity, Vector3.zero, Time.fixedDeltaTime * 0.5f);
            }
            else
            {
                // 正常飞行时的速度限制
                Vector3 horizontalVelocity = new Vector3(velocity.x, 0, velocity.z);
                float maxVerticalSpeed = horizontalVelocity.magnitude * 0.5f;
            ...
            if (velocity.magnitude > 0.1f)
            {
                ...
                if (isHovering)
                {
                    // 悬停时朝向悬停目标点
                    turnSpeed = 2f;
                    targetDirection = (beeTarget.HoverCenter + Vector3.up * beeTarget.hoverHeight - transform.position).normalized;
                }
                else
                {
                    // 正常飞行时朝向速度方向
                    turnSpeed = isAvoidingObstacle ? 4.5f : 3f;
                    targetDirection = velocity.normalized;
                }

                if (targetDirection.magnitude > 0.1f)
                {
                    transform.forward = Vector3.Lerp(transform.forward, targetDirection, Time.fixedDeltaTime * turnSpeed);
                }
            }
        ...
        private Vector3 CalculateGroundAvoidanceForce()
        {
            ...
            // 检测地面距离
            if (Physics.Raycast(transform.position, Vector3.down, out groundHit, groundAvoidanceHeight * 2f, groundLayerMask))
            {
                groundDistance = groundHit.distance;
                nearGround = groundDistance < groundAvoidanceHeight;
            }

            if (!nearGround) return Vector3.zero;

            // 计算地面避免力 - 向上且稍微向前
            float avoidanceStrength = 1.0f - (groundDistance / groundAvoidanceHeight);
            Vector3 upwardForce = Vector3.up * avoidanceStrength * groundAvoidanceWeight;

            // 添加向前分量，帮助蜜蜂离开地面
            Vector3 forwardForce = transform.forward * avoidanceStrength * groundAvoidanceWeight * 0.5f;

            // 可视化
            if (showForces)
            {
                Debug.DrawRay(transform.position, upwardForce, Color.magenta);
                Debug.DrawRay(transform.position, forwardForce, Color.cyan);
            }

            return upwardForce + forwardForce;
        }
        ...
        private Vector3 CalculateOscillationForce()
        {
            ...
            // 改进的升力计算 - 添加高度稳定机制
            float targetHeight = 10f; // 目标飞行高度
            float currentHeight = transform.position.y;
            float heightError = targetHeight - currentHeight;

            // 高度修正力 - 使蜜蜂趋向目标高度
            float heightCorrection = Mathf.Clamp(heightError * 0.5f, -3f, 3f);

            float baseLift = rb.mass * 1.2f; // 基础升力略大于重力
            ...
            if (showForces)
            {
                Debug.DrawRay(transform.position, liftForce.normalized, Color.green);
            ...
            return (liftForce + thrustForce + swayForce) * 0.6f + horizontalPerturbation;
        }
        //#region 身体振荡力
        /// <summary>
        ...
        private void ApplyBodyPosture()
        {
            ...
            if (velocity.magnitude > 0.1f)
            {
                ...
                {
                    horizontalVelocity = transform.forward; // 备用方向
                }

                Quaternion forwardRotation = Quaternion.LookRotation(horizontalVelocity.normalized);

                // 应用限制后的身体角度
                float finalPitchAngle = Mathf.Clamp(bodyPitchAngle, -40f, 40f);
                float finalRollAngle = Mathf.Clamp(bodyRollAngle, -50f, 50f);

                // 在避障状态下增强翻滚效果
                if (isAvoidingObstacle && enableAvoidanceRoll)
                {
                    float avoidanceRollContribution = currentAvoidanceRollAngle;
                    finalRollAngle = bodyRollAngle * 0.5f + avoidanceRollContribution * 1.2f;
                    finalRollAngle = Mathf.Clamp(finalRollAngle, -avoidanceRollMaxAngle, avoidanceRollMaxAngle);
                }
                ...
                // 组合所有旋转
                Quaternion targetRotation = forwardRotation * pitchRotation * rollRotation;

                // 根据避障状态调整旋转响应速度
                float rotationSpeed = isAvoidingObstacle ? 5f : 3f;
